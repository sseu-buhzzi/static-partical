...
class ParticalList{
    ...
    update_partical(index) {
        let partical = this.list[index];
        let ddx = [0, 0, 0];
    ...
    update() {
        ...
        if(!false)for (let partical of this.list) {
            let ddx = [0, 0, 0];
            for (let other_partical of this.list) {
                if (partical === other_partical) {
                    continue;
            ...
            partical.dx[1] = decay * partical.dx[1] + (ddx[1] || 0);
            partical.dx[2] = decay * partical.dx[2] + (ddx[2] || 0);
        }else for(let i=0;i<this.list.length;++i)this.update_partical(i);
        this.display(true);
    }
